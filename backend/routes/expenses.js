...
router.post('/', authenticate, (req, res) => {
  try {
    const { type, category, amount, currency, date, description, bl_number, supervisor_name, company_warehouse, province_city } = req.body;
    if (!type || !amount || !date || !description) return res.status(400).json({ error: 'Thiếu thông tin bắt buộc' });
    const result = db.prepare('INSERT INTO expenses (type,category,amount,currency,date,description,bl_number,supervisor_name,company_warehouse,province_city,submitted_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)').run(type, category || 'other', amount, currency || 'VND', date, description, bl_number||null, supervisor_name||null, company_warehouse||null, province_city||null, req.user.id);
...
router.put('/:id', authenticate, (req, res) => {
  try {
    const { type, category, amount, currency, date, description, status, bl_number, supervisor_name, company_warehouse, province_city } = req.body;
    ...
    db.prepare('UPDATE expenses SET type=?,category=?,amount=?,currency=?,date=?,description=?,bl_number=?,supervisor_name=?,company_warehouse=?,province_city=?,status=?,approved_by=? WHERE id=?').run(type, category, amount, currency, date, description, bl_number||null, supervisor_name||null, company_warehouse||null, province_city||null, status || expense.status, approvedBy, req.params.id);
